...
const EM = [
  ...
  { idx: 3, string: 3, label: "G", note: "G3",  freq: 196.00, fret: null, wound: false, thickness: 1.6 },
...
function fftChordStatus(analyser, sampleRate) {
  ...
  const result    = {};
  for (const s of EM) {
    const center = Math.round(s.freq / binW);
    const half   = Math.ceil(TOL / binW);
    let maxDb    = -Infinity;
    for (let i = Math.max(0, center - half); i <= Math.min(buf.length - 1, center + half); i++) {
      if (buf[i] > maxDb) maxDb = buf[i];
    }
    const suspect = confirmed.some(f => HARM_RATIOS.some(r => Math.abs(f * r - s.freq) < TOL));
    const thresh  = suspect ? BASE_DB + HARM_BOOST : BASE_DB;
    const ringing = maxDb > thresh;
    if (ringing) confirmed.push(s.freq);
    result[s.string] = ringing;
  }
  return result;
}
...
export default function StringPractice({ onBack }) {
    ...
    try {
      if (Platform.OS === "web") {
        if (!navigator?.mediaDevices?.getUserMedia) {
          setMicErr("Microphone not supported in this browser."); return;
        }
        ...
        const anlz     = ctx.createAnalyser();
        anlz.fftSize               = 8192;
        anlz.smoothingTimeConstant = 0.72;
        src.connect(anlz);
        ctxRef.current  = ctx;
        anlzRef.current = anlz;
      } else {
        const { granted } = await Audio.requestPermissionsAsync();
        if (!granted) { setMicErr("Microphone permission denied."); return; }
      ...
      okRef.current    = 0;
      setPhase("strings"); setCurIdx(0); setConfCount(0);
      setDoneMask([]); setChordStatus({}); setOkFrames(0);
      beginPolling();
    } catch (e) {
      setMicErr(e.message || "Failed to start microphone.");
    }
  }

  function beginPolling() {
    pollRef.current = setInterval(() => {
      const anlz = anlzRef.current;
      const ctx  = ctxRef.current;
      if (!anlz || !ctx) return;

      if (phaseRef.current === "strings") {
        const tbuf = new Float32Array(anlz.fftSize);
        anlz.getFloatTimeDomainData(tbuf);
        ...
        if (confRef.current >= CONF_NEED) {
          ...
          confRef.current = 0; setConfCount(0);
          if (nextIdx >= EM.length) {
            phaseRef.current = "chord"; setPhase("chord"); okRef.current = 0;
          } else {
            idxRef.current = nextIdx; setCurIdx(nextIdx);
          }
        }
      } else if (phaseRef.current === "chord") {
        const status = fftChordStatus(anlz, ctx.sampleRate);
        setChordStatus(status);
        const allOk = EM.every(s => status[s.string]);
        if (allOk) {
          okRef.current++;
          setOkFrames(okRef.current);
          if (okRef.current >= 4) { phaseRef.current = "done"; setPhase("done"); }
        } else {
          okRef.current = 0; setOkFrames(0);
        }
      }
    }, 55);
  }
  ...
  function stopMic() {
    ...
    streamRef.current = null; anlzRef.current = null;
  }

  function reset() {
    stopMic();
    setPhase("idle"); setCurIdx(0); setConfCount(0);
    setDoneMask([]); setChordStatus({}); setOkFrames(0); setMicErr("");
    confRef.current = 0; idxRef.current = 0;
    phaseRef.current = "strings"; okRef.current = 0;
  }
    ...
    <View style={sp.container}>
      <View style={sp.header}>
        ...
        <Text style={sp.headerTitle}>
          {phase === "idle" ? "String Practice"
            : inStrings     ? `String ${curIdx + 1} of 6`
            : inChord       ? "Strum Full Chord"
            : "Complete! 🎸"}
        </Text>
      </View>
          ...
          return (
            <View key={s.string} style={[sp.stringRow, { backgroundColor: rowBg }]}>
              ...
              <View style={sp.trackWrap}>
                ...
                {current && progress > 0 && (
                  <View style={[sp.progFill, { width: `${progress * 100}%` }]} />
                )}
              </View>
...
const sp = StyleSheet.create({
  ...
  fretBox:     { position: "absolute", right: 0, backgroundColor: "#2a1c0a", borderRadius: 6, paddingHorizontal: 6, paddingVertical: 2, borderWidth: 1, borderColor: "#987D30" },
